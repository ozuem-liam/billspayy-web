...
function DataPageInner() {
  ...
  const handlePinComplete = async (pin: string) => {
    ...
    try {
      ...
      router.push(
        `/pay/result?reference=${reference}&status=${status}&amount=${amountKobo}&category=DATA&recipient=${phone}&network=${network}`
      )
    } catch (error: any) {
      toast.dismiss('data-pay')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-white p-6 shadow-sm border border-gray-100 space-y-6">
        ...
        {selectedPlan && hasEnoughBalance && (
          <CommissionPreview amount={selectedPlan.amount} category="DATA" provider={network ?? undefined} />
        )}
      </div>
